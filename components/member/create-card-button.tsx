...
import { CARD_PRICE_IN_EURO_CENTS } from "@/lib/config";
...
export default function CreateCardButton({
  ...
  const warnings = useMemo(() => {
    ...
    if (members.some((member) => !member.imagePath?.length)) {
      array = [
        ...array,
    ...
    if (!organization.imagePath?.length) {
      array = [
        ...array,
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    const result = await executeAsync({
      ...
      cardVisualId: values.cardVisualId,
    });
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent>
        <SheetHeader>
                      ...
                      <FormItem>
                        ...
                        <div className="col-span-3">
                          ...
                          >
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue placeholder="Sélectionnez un modèle de carte" />
                              </SelectTrigger>
                            </FormControl>
                            <SelectContent>
                              {cardVisuals.map((cardVisual) => (
                                <SelectItem
                                  key={cardVisual.id}
                                  value={cardVisual.id}
                                >
          ...
          {!cardVisuals.length && (
            <div>
              <Link href={`/organizations/${organization.id}/settings/visual`}>
                <Button className="w-full">Ajouter un modèle de carte</Button>
              </Link>

              <p className="text-sm text-red-500 text-center mt-5 font-bold">
                Vous devez ajouter un modèle de carte avant de pouvoir commander
                des cartes.
              </p>
            </div>
          )}
